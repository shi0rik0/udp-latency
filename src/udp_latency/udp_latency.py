...
class Client:
    ...
    def synchronize(self, verbose: bool) -> None:
        ...
        for _ in range(10):
            ...
            _send_nums = self._udp_socket.sendto(
    ...
    def send(
        ...
        if packet_size < HEADER_SIZE or packet_size > 1500:
            raise Exception(
                "warning: packet size should be no larger than 1500 bytes.")
        ...
        while True:
            ...
            if dyna:
                prac_period = ((running_time - (current_time - start_time)) /
                               ...
                               (len(self.log) /
                                (frequency *
                                 (current_time - start_time + 1) * 1e-9)) *
                               1e-9)
...
class Server:
    ...
    def listen(self, buffer_size: int, verbose: bool, sync: bool):
        ...
        while True:
            ...
            old_latency = latency
            latency = round(
                float(recv_time - send_time) * 1e-9 - float(self.OFFSET), 6)
